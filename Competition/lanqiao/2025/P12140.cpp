...
int n, m;
// 返回获得的积分
int check(int xa, int xb, int xc)
{
    if(a[xa] == b[xb] && b[xb] == c[xc])
        return 200;
    else if(a[xa]+1 == b[xb] && b[xb]+1 == c[xc])
        return 200;
    else if(a[xa] == b[xb] || b[xb] == c[xc] || a[xa]==c[xc])
        return 100;
    else if(max(max(a[xa], b[xb]), c[xc])-min(min(a[xa], b[xb]), c[xc]) == 2)
        return 100;
    else
        return 0;
}
