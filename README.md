...
## Trie
适用于字符种类较少而数量较多且需要高效存储和查找字符串
- son中下标`[0][0]`的点既是根结点,也是空结点；cnt中下标`[0]`的点也是空结点
```cpp
int son[N][M]; //字符串总共结点容量为N,有M种字符。存储每个结点处对应的idx
int cnt[C];//所有结点的单词数，C=M*N
int idx;//当前用到的节点
// 插入
...
```
## 并查集
适用于多个集合执行集合合并操作并且询问两个元素是否在同一个集合中
- p存储每个编号的父节点
```cpp
int p[N];
